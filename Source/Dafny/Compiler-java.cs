...
namespace Microsoft.Dafny{
  public class JavaCompiler : Compiler{
    ...
    protected void DeclareField(string name, bool isStatic, bool isConst, Type type, Bpl.IToken tok, string rhs, ClassWriter cw) {
      if (isStatic) {
        var r = rhs ?? DefaultValue(type, cw.StaticMemberWriter, tok, Usage.Ordinary);
        ...
        cw.CtorBodyWriter.WriteLine("this.{0} = {1};", name, rhs ?? PlaceboValue(type, cw.CtorBodyWriter, tok, Usage.Ordinary));
      }
    }
        ...
        switch (AsJavaNativeType(enclosingType)) {
          ...
          case null:
            if (enclosingType.IsBoolType) {
              ...
              var d = DefaultValue(enclosingType, wr, enclosingType.tok, Usage.Ordinary);
    ...
    IClassWriter CompileDatatypeBase(DatatypeDecl dt, TargetWriter wr) {
      ...
      if (dt.TypeArgs.Count == 0) {
        ...
        wDefault = wr.Fork();
        wr.WriteLine(";");
        using (var w = wr.NewBlock($"public static {DtT_protected} Default()")) {
      ...
      arguments = Util.Comma(usedTypeArgs, tp => DefaultValue(new UserDefinedType(tp), wDefault, dt.tok, Usage.Ordinary, true));
    ...
    protected override string TypeInitializationValue(Type type, TextWriter wr, Bpl.IToken tok, Usage usage, bool usePlaceboValue, bool constructTypeParameterDefaultsFromTypeDescriptors) {
      ...
      if (cl is NewtypeDecl) {
        ...
        return $"{s}.{typeargs}Default({Util.Comma(relevantTypeArgs, ta => DefaultValue(ta.Actual, wr, tok, Usage.Ordinary, constructTypeParameterDefaultsFromTypeDescriptors))})";
      } else {
        Contract.Assert(false);
    ...
    private void CreateDafnyArrays(int i, TargetWriter outputWr) {
      ...
      using (var wrBody = wr.NewBlock($"public Array{i}({DafnyTypeDescriptor}<T> elmtType, {Util.Comma(dims, j => $"int dim{j}")}, Object{outerBrackets} elmts)")) {
    ...
    protected override void EmitNewArray(Type elmtType, Bpl.IToken tok, List<Expression> dimensions, bool mustInitialize, TargetWriter wr) {
      ...
      if (dimensions.Count > 1) {
        ...
        wr.Write(")");
        if (mustInitialize) {
          wr.Write($".fillThenReturn({DefaultValue(elmtType, wr, tok, Usage.Ordinary, true)})");
        }
      } else {
        if (!elmtType.IsTypeParameter) {
