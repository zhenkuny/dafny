...
namespace Microsoft.Dafny
{
  ...
  public class RefinementTransformer : IRewriter
  {
    ...
    Function CloneFunction(IToken tok, Function f, Usage usage, List<MaybeFreeExpression> moreEnsures, Formal moreResult, Expression moreBody, Expression replacementBody, bool checkPrevPostconditions, Attributes moreAttributes) {
      Contract.Requires(tok != null);
      ...
      if (f is Predicate) {
        return new Predicate(tok, f.Name, f.HasStaticKeyword, f.IsProtected, usage, tps, formals,
        ...
        return new Function(tok, f.Name, f.HasStaticKeyword, f.IsProtected, usage, tps, formals, result, refinementCloner.CloneType(f.ResultType),
          req, reads, ens, decreases, body, refinementCloner.MergeAttributes(f.Attributes, moreAttributes), null);
      }
    }
    ...
    Method CloneMethod(Method m, List<MaybeFreeExpression> moreEnsures, Specification<Expression> decreases, BlockStmt newBody, bool checkPreviousPostconditions, Attributes moreAttributes) {
      ...
      if (m is InductiveLemma) {
        ...
        return new Method(new RefinementToken(m.tok, moduleUnderConstruction), m.Name, m.HasStaticKeyword, m.Usage, m.HasInoutThis, tps, ins, m.Outs.ConvertAll(refinementCloner.CloneFormal),
          req, mod, ens, decreases, body, refinementCloner.MergeAttributes(m.Attributes, moreAttributes), null);
      }
    }
    ...
    public void CheckOverrideResolvedParameters(IToken tok, List<Formal> old, List<Formal> nw, string name, string thing, string parameterKind)
    {
        ...
        else
        {
            for (int i = 0; i < old.Count; i++)
            {
                ...
                var n = nw[i];
                if (o.Usage != n.Usage) {
                  reporter.Error(MessageSource.RefinementTransformer, n.tok, "{0} '{1}' of {2} {3} cannot be changed, compared to the corresponding {2} in the module it overrides, from {4} to {5}", parameterKind, n.Name, thing, name,
