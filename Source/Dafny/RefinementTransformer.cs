      ...
      if (d is ModuleDecl) {
        if (!(nw is ModuleDecl)) {
          ...
          if (!CheckIsRefinement((ModuleDecl)nw, (ModuleFacadeDecl)d))
            reporter.Error(MessageSource.RefinementTransformer, nw.tok, "a module ({0}) can only be replaced by a refinement of the original module. New is {1}", d.Name, nw.Name);
        }
