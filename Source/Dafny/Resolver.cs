  ...
  public class Resolver {
    ...
    class AmbiguousMemberDecl : MemberDecl, IAmbiguousThing<MemberDecl> // only used with "classes"
    {
      ...
      private AmbiguousMemberDecl(ModuleDefinition m, string name, ISet<MemberDecl> pool)
        : base(pool.First().tok, name, true, pool.First().Usage, null, false) {
        Contract.Requires(name != null);
        Contract.Requires(pool != null && 2 <= pool.Count);
        Pool = pool;
      }
    ...
    private void BindModuleName_LiteralModuleDecl(LiteralModuleDecl litmod,
      ...
      // TODO: This should take the graph as input and recursively add dependencies on the
      //       functors and arguments that it finds.  We'll only resolve the functors'
      //       definitions at this time, not the result of applying the functor,
      //       since we're not yet far enough into resolution to have all of the necessary info.
      //       That means no reading/writing to this.virtualModules
    private bool ResolveQualifiedModuleIdRootRefines(ModuleDefinition context, ModuleBindings bindings, ModuleQualifiedId qid,
      ...
      if (qid.FunctorApp != null) {
        if (qid.FunctorApp.functor == null) {
          // Find the definition of this functor
          ModuleDecl functorDecl = null;
          bool res = bindings.TryLookupFilter(qid.FunctorApp.tok.val, out functorDecl, m => m.EnclosingModuleDefinition != context);
          if (!res) {
            return false;
          }
          ...
          foreach (ModuleQualifiedId id in qid.FunctorApp.moduleParamNames) {
            ...
            bool res = ResolveQualifiedModuleIdRootRefines(context, bindings, id, out moduleParamDecl);
            qid.FunctorApp.moduleParams.Add();
          }

          // Compute (and cache) the result of doing the functor application
        }
        qid.Root = this.virtualModules[qid.FunctorApp];
      } else {
    ...
    private void ProcessDependenciesDefinition(ModuleDecl decl, ModuleDefinition m, ModuleBindings bindings,
      ...
      if (m is Functor f) {
        foreach (ModuleFormal formal in f.Formals) {
          ...
          bool res = ResolveQualifiedModuleIdRootRefines(((LiteralModuleDecl)decl).ModuleDef, bindings, formal.TypeName, out other);
          if (!res) {
...
//        if (!bindings.TryLookupFilter(alias.TargetQId.rootToken(), out root, m => alias != m)
    ...
    public void ResolveTopLevelDecls_Core(List<TopLevelDecl/*!*/>/*!*/ declarations, Graph<IndDatatypeDecl/*!*/>/*!*/ datatypeDependencies, Graph<CoDatatypeDecl/*!*/>/*!*/ codatatypeDependencies, bool isAnExport = false) {
      ...
      foreach (TopLevelDecl topd in declarations) {
        ...
        if (d is NewtypeDecl) {
          var dd = (NewtypeDecl)d;
          ResolveAttributes(d.Attributes, d, new ResolveOpts(new NoContext(d.EnclosingModuleDefinition), false));
            ...
            if ((e is SetComprehension && ((SetComprehension)e).Finite) || (e is MapComprehension && ((MapComprehension)e).Finite)) {
              ...
              if (e.Type.HasFinitePossibleValues) {
                ...
                foreach (var bv in ComprehensionExpr.BoundedPool.MissingBounds(e.BoundVars, e.Bounds, ComprehensionExpr.BoundedPool.PoolVirtues.Finite)) {
                  resolver.reporter.Error(MessageSource.Resolver, e, "the result of a {0} must be finite, but Dafny's heuristics can't figure out how to produce a bounded set of values for '{1}'", what, bv.Name);
                }
              }
            }
          }
...
#region CheckDividedConstructorInit
    ...
    private void LetBind(RBranch branch, IdPattern var, Expression genExpr) {
      ...
      if (branch is RBranchStmt branchStmt) {
        ...
        var cLet = new VarDeclPattern(cLVar.Tok, cLVar.Tok, cPat, expr, usage);
    ...
    private void CompileNestedMatchExpr(NestedMatchExpr e, ResolveOpts opts) {
      ...
      MatchTempInfo mti = new MatchTempInfo(e.tok, e.Cases.Count(), opts.codeContext, e.Usage, DafnyOptions.O.MatchCompilerDebug);
    ...
    */
    ...
    private void ResolveAssignOrReturnStmt(AssignOrReturnStmt s, ICodeContext codeContext) {
      ...
      var lhss = new List<LocalVariable>() { new LocalVariable(s.Tok, s.Tok, temp, new InferredTypeProxy(), Usage.Ordinary) };
      // "var temp ;"
      ...
      if (callee.Ins.Count != s.Args.Count) {
        ...
        foreach (var (fInout, aInout, idx) in System.Linq.Enumerable.Zip(callee.Ins, s.Args)
          ...
          if (fInout) {
            ...
            Contract.Assert(aInout);
            reporter.Error(MessageSource.Resolver, s, "method in-parameter {0} should not be inout", idx);
          }
        }
    ...
    public static Type SubstType(Type type, Dictionary<TypeParameter, Type> subst) {
      ...
      if (type is BasicType) {
        ...
        return new ArrowType(t.tok, (ArrowTypeDecl)t.ResolvedClass, t.Args.ConvertAll(u => SubstType(u, subst)), SubstType(t.Result, subst), t.ResultUsage, t.Usages);
      } else if (type is UserDefinedType) {
        var t = (UserDefinedType)type;
    ...
    public void ResolveExpressionX(Expression expr, ResolveOpts opts) {
      ...
      if (expr is ParensExpression) {
        ...
        var arrowType = builtIns.CreateArrowType(e.tok, true, Arrow.Any, new List<Type>() { t }, elementType, Usage.Ordinary, null);
