...
#region first procedure, no impl yet
      ...
      // Now for the other branch, where the postcondition of the call is exported.
      {
        ...
        if (forallExpressions != null) {
          ...
          for (int i = 0; i < s0.Method.Ins.Count; i++) {
            var arg = Substitute(s0.Args[i].Expr, null, substMap, s0.MethodSelect.TypeArgumentSubstitutionsWithParents());  // substitute the renamed bound variables for the declared ones
    ...
    void TrCallStmt(CallStmt s, BoogieStmtListBuilder builder, List<Variable> locals, ExpressionTranslator etran, Bpl.IdentifierExpr actualReceiver) {
      ...
      ProcessCallStmt(s.Tok, tySubst, GetTypeParams(s.Method), s.Receiver, actualReceiver, s.Method, s.MethodSelect.AtLabel, s.Args.ConvertAll(a => a.Expr), bLhss, lhsTypes, builder, locals, etran);
    ...
    void ProcessCallStmt(IToken tok,
      ...
      for (int i = 0; i < callee.Ins.Count; i++) {
        ...
        var local = new LocalVariable(formal.tok, formal.tok, formal.Name + "#", Resolver.SubstType(formal.Type, tySubst), formal.Usage);
        local.type = local.OptionalType;  // resolve local here
      ...
      public virtual Expression Substitute(Expression expr) {
        ...
        if (expr is StaticReceiverExpr) {
          ...
          if (e.Exact) {
            ...
            var newLet = new SubstLetExpr(e.tok, e.LHSs, new List<Expression>{ rhs }, body, e.Exact, e.Usage, e, newSubstMap, newTypeMap, newBounds);
            newExpr = newLet;
          }
        } else if (expr is MatchExpr) {
          var e = (MatchExpr)expr;
      ...
      protected virtual List<LocalVariable> CreateLocalVarSubstitutions(List<LocalVariable> vars, bool forceSubstitutionOfVars) {
        ...
        foreach (var v in vars) {
          ...
          if (!forceSubstitutionOfVars && tt == v.OptionalType) {
            ...
            anythingChanged = true;
            var newVar = new LocalVariable(v.Tok, v.EndTok, v.Name, tt, v.Usage);
