  ...
  class Cloner
  {
    ...
    public virtual TopLevelDecl CloneDeclaration(TopLevelDecl d, ModuleDefinition m) {
      ...
      if (d is OpaqueTypeDecl) {
        ...
        return new TupleTypeDecl(dd.Usages, dd.EnclosingModuleDefinition, dd.Attributes);
      } else if (d is IndDatatypeDecl) {
        var dd = (IndDatatypeDecl)d;
        var tps = dd.TypeArgs.ConvertAll(CloneTypeParam);
        var ctors = dd.Ctors.ConvertAll(CloneCtor);
        var dt = new IndDatatypeDecl(Tok(dd.tok), dd.Name, m, tps, ctors, dd.Members.ConvertAll(CloneMember), CloneAttributes(dd.Attributes), dd.IsLinear, dd.IsRefining);
        return dt;
    ...
    public virtual Statement CloneStmt(Statement stmt) {
      ...
      if (stmt is AssertStmt) {
        ...
        r = new VarDeclPattern(Tok(s.Tok), Tok(s.EndTok), CloneCasePattern(s.LHS), CloneExpr(s.RHS), s.Usage, s.IsAutoGhost);
    ...
    public virtual Function CloneFunction(Function f, string newName = null) {
      ...
      if (f is Predicate) {
        return new Predicate(Tok(f.tok), newName, f.HasStaticKeyword, f.IsGhost, f.Usage, tps, formals,
        ...
        return new Function(Tok(f.tok), newName, f.HasStaticKeyword, f.IsGhost, f.Usage, tps, formals, f.Result == null ? null : CloneFormal(f.Result), CloneType(f.ResultType),
          req, reads, ens, decreases, body, CloneAttributes(f.Attributes), null);
      }
    }
